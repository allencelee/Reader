        ...
        if let tab = try? Database.shared.viewContext.existingObject(with: tabID) as? Tab {
            ...
            Task { [weak self] in
                await MainActor.run { [weak self] in
                    ...
                    if let webURL = self?.webView.url, webURL.isKpappURL {
    ...
    func webView(
        ...
        Task { @MainActor in
            ...
            (webView.configuration
                .urlSchemeHandler(forURLScheme: KpappURLSchemeHandler.ZIMScheme) as? KpappURLSchemeHandler)?
                .didFailProvisionalNavigation()
        }
        guard error.code != NSURLErrorCancelled else { return }
        guard canShowMimeType else {
            guard let kpappURL = error.userInfo[NSURLErrorFailingURLErrorKey] as? URL else {
                return
            }
            NotificationCenter.saveContent(url: kpappURL)
            return
        }
        NotificationCenter.default.post(
